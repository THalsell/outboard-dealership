...
export default function InventoryPageClient() {
  const { filters, resetFilters, updateFilter } = useFilter();
  ...
  useEffect(() => {
    ...
    if (urlFilters.brand) {
      ...
      if (
        ...
        normalizedBrand =
          urlFilters.brand.charAt(0).toUpperCase() +
          urlFilters.brand.slice(1).toLowerCase();
      }
    ...
    filtered = filtered.filter((product) => {
      ...
      if (filters.trimTilt.length > 0) {
        ...
        if (trimTiltSpec) {
          ...
          if (specLower.includes("power trim") && specLower.includes("tilt")) {
            ...
            specLower.includes("power") &&
            specLower.includes("tilt")
          ) {
            productTrimTiltType = "power-tilt";
          } else if (
            specLower.includes("manual") ||
            specLower.includes("6") ||
            specLower.includes("5")
          ) {
      ...
      if (filters.steering.length > 0) {
        ...
        if (!productSteeringType) {
          ...
          if (
            product.tags.some((tag) => remoteTags.includes(tag.toLowerCase()))
          ) {
            productSteeringType = "remote";
          } else if (
            product.tags.some((tag) => tillerTags.includes(tag.toLowerCase()))
          ) {
            productSteeringType = "tiller";
          }
        }
      ...
      if (filters.starting.length > 0) {
        ...
        if (startingSpec) {
          ...
          if (
            specLower.includes("electric") ||
            specLower.includes("key") ||
            specLower.includes("button")
          ) {
            productStartType = "electric";
          } else if (
            specLower.includes("manual") ||
            specLower.includes("pull") ||
            specLower.includes("recoil")
          ) {
        ...
        if (!productStartType) {
          ...
          if (
            product.tags.some((tag) => electricTags.includes(tag.toLowerCase()))
          ) {
            productStartType = "electric";
          } else if (
            product.tags.some((tag) => manualTags.includes(tag.toLowerCase()))
          ) {
            productStartType = "manual";
          }
        }
      ...
      if (filters.fuelTank.length > 0) {
        ...
        // Check metafield first (same as specs system)
        const fuelTankSpec =
          product.specs?.["Fuel Tank Type"] ||
          product.specs?.["fuel.tank_type"];

        if (fuelTankSpec) {
          const specLower = fuelTankSpec.toLowerCase();
          if (
            specLower.includes("internal") ||
            specLower.includes("built-in") ||
            specLower.includes("integrated")
          ) {
            productTankType = "internal";
          } else if (
            specLower.includes("external") ||
            specLower.includes("portable") ||
            specLower.includes("remote")
          ) {
        ...
        if (!productTankType) {
          ...
          if (
            product.tags.some((tag) => internalTags.includes(tag.toLowerCase()))
          ) {
            productTankType = "internal";
          } else if (
            product.tags.some((tag) => externalTags.includes(tag.toLowerCase()))
          ) {
            productTankType = "external";
          }
        }
      ...
      if (filters.fuelDelivery.length > 0) {
        ...
        // Check metafield first (same as specs system)
        const fuelDeliverySpec =
          product.specs?.["Fuel Induction System"] ||
        ...
        if (fuelDeliverySpec) {
          ...
          if (
            specLower.includes("efi") ||
            specLower.includes("injection") ||
            specLower.includes("electronic")
          ) {
            productFuelType = "efi";
          } else if (
            specLower.includes("carburetor") ||
            specLower.includes("carb")
          ) {
            productFuelType = "carburetor";
          } else if (
            specLower.includes("propane") ||
            specLower.includes("lpg")
          ) {
        ...
        if (!productFuelType) {
          ...
          if (product.tags.some((tag) => efiTags.includes(tag.toLowerCase()))) {
            ...
            product.tags.some((tag) => carbTags.includes(tag.toLowerCase()))
          ) {
            productFuelType = "carburetor";
          } else if (
            product.tags.some((tag) => propaneTags.includes(tag.toLowerCase()))
          ) {
            productFuelType = "propane";
          }
        }
      ...
      if (filters.shaftLengths.length > 0) {
        ...
        const productShaftLengths = product.variants
          .filter((v) => {
            ...
            return (
              (optionName.includes("shaft") || optionName.includes("length")) &&
              v.option1Value
            );
          })
          .map((v) => {
    ...
    switch (filters.sortBy) {
      ...
      default: // 'featured'
        filtered.sort((a, b) => {
          ...
          const aInventory = a.variants.reduce(
            (sum, v) => sum + v.inventory,
            0
          );
          const bInventory = b.variants.reduce(
            (sum, v) => sum + v.inventory,
            0
          );
  ...
  const dynamicRanges = useMemo(() => {
    ...
    allProducts.forEach((product) => {
      ...
      if (product.specs) {
        const shaftLengthSpec =
          product.specs["Shaft Length"] ||
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Mobile Filter Page */}
      {showMobileFilters && (
        <div className="fixed inset-0 top-[156px] sm:top-[104px] z-50 lg:hidden bg-white overflow-y-auto">
          <FilterSidebar
            isMobile={true}
      ...
      <div className="container mx-auto px-4 pb-6 pt-[80px] sm:pt-[60px]">
