...
export async function POST(request: Request) {
  try {
    ...
    const searchResponse = await fetch(
      `https://${SHOPIFY_STORE_DOMAIN}/admin/api/2024-10/customers/search.json?query=email:${encodeURIComponent(
        email
      )}`,
      {
        method: "GET",
    ...
    if (!searchResponse.ok) {
      ...
      throw new Error(
        `Shopify search error: ${searchResponse.status} - ${errorData}`
      );
    }
    ...
    if (!createResponse.ok) {
      ...
      throw new Error(
        `Failed to create customer: ${JSON.stringify(errorData)}`
      );
    }
