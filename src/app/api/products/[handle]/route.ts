...
function transformDetailedGraphQLProduct(
  ...
  const images =
    ...
    Array.isArray(
      ...
      ? (
          ...
          (
            edge: { node: { url: string; altText?: string } },
            index: number
          ) => ({
            src: edge.node.url,
            position: index + 1,
  ...
  const variants =
    graphqlProduct.variants && Array.isArray(graphqlProduct.variants.edges)
      ? graphqlProduct.variants.edges.map(
          (edge: {
            node: {
              id: string;
              sku?: string;
  ...
  const metafieldsArray = Array.isArray(graphqlProduct.metafields)
    ...
    : graphqlProduct.metafields?.edges?.map(
        (edge: {
          node: {
            namespace: string;
            key: string;
...
export async function GET(
  ...
  try {
    ...
    const product = transformDetailedGraphQLProduct(
      graphqlProduct as GraphQLProduct
    );
