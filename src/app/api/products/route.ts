...
function transformGraphQLProduct(graphqlProduct: GraphQLProduct): Product {
  ...
  if (hpMatch) {
    ...
    if (
      modelMatch &&
      parseInt(modelMatch[1]) > 1 &&
      parseInt(modelMatch[1]) < 500
    ) {
  ...
  // Get main image
  const images =
    graphqlProduct.images?.edges?.map(
  ...
  if (graphqlProduct.metafields && Array.isArray(graphqlProduct.metafields)) {
    graphqlProduct.metafields.forEach(
      (metafield: { namespace?: string; key: string; value: string }) => {
        if (metafield && metafield.value && metafield.key) {
          ...
          if (
            ...
            const specKey = getSpecificationKey(
              metafield.namespace,
              metafield.key
            );
            if (specKey) {
              ...
              // Also add the raw key with cleaned format
              const rawKey = `${metafield.namespace || "unknown"}.${
                metafield.key
              }`;
              specs[rawKey] = metafield.value;
            }
          }
        }
      }
  ...
  const variants =
    graphqlProduct.variants?.edges?.map((edge: VariantEdge) => {
      ...
      if (variant.selectedOptions && variant.selectedOptions.length > 0) {
        ...
        const shaftOption = variant.selectedOptions.find(
          (opt) =>
...
export async function GET(request: Request) {
  try {
    ...
    const brand = searchParams.get("brand"); // Get brand filter
    const minPrice = searchParams.get("minPrice"); // Get min price filter
    const maxPrice = searchParams.get("maxPrice"); // Get max price filter
    const minHorsepower = searchParams.get("minHorsepower"); // Get min HP filter
    const maxHorsepower = searchParams.get("maxHorsepower"); // Get max HP filter

    // Build Shopify query from parameters
    let shopifyQuery = query
      ? `title:*${query}* OR description:*${query}* OR tags:${query}`
      : "";

    // Add brand filter to Shopify query
    if (brand) {
      // Try both the original case and proper case
      const brandProperCase = brand.charAt(0).toUpperCase() + brand.slice(1).toLowerCase();
      // Use title search instead of brand: since Shopify's brand: field doesn't work with metafields
      const brandQuery = `title:*${brandProperCase}*`;
      shopifyQuery = shopifyQuery
        ? `(${shopifyQuery}) AND ${brandQuery}`
        : brandQuery;
    }

    // Add status filters to Shopify query
    if (status === "overstock") {
      const statusQuery = "tag:overstock";
      shopifyQuery = shopifyQuery
        ? `(${shopifyQuery}) AND ${statusQuery}`
        : statusQuery;
    } else if (status === "sale") {
      const statusQuery = "tag:sale OR tag:on-sale OR tag:special";
      shopifyQuery = shopifyQuery
        ? `(${shopifyQuery}) AND ${statusQuery}`
        : statusQuery;
    }

    // Add condition filter to Shopify query
    if (condition) {
      const conditionQuery = `tag:${condition}`;
      shopifyQuery = shopifyQuery
        ? `(${shopifyQuery}) AND ${conditionQuery}`
        : conditionQuery;
    }

    // Add price range filters to Shopify query
    if (minPrice) {
      const priceQuery = `variants.price:>=${minPrice}`;
      shopifyQuery = shopifyQuery
        ? `(${shopifyQuery}) AND ${priceQuery}`
        : priceQuery;
    }
    if (maxPrice) {
      const priceQuery = `variants.price:<=${maxPrice}`;
      shopifyQuery = shopifyQuery
        ? `(${shopifyQuery}) AND ${priceQuery}`
        : priceQuery;
    }
    ...
    // Fetch products using GraphQL Storefront API with built query
    const graphqlProducts = (await fetchProducts(
      shopifyQuery,
      limit
    )) as GraphQLProduct[];
    ...
    // Apply additional server-side filters that couldn't be handled in Shopify query

    // Filter by horsepower range (can't be done in Shopify query)
    if (minHorsepower || maxHorsepower) {
      const minHP = minHorsepower ? parseFloat(minHorsepower) : 0;
      const maxHP = maxHorsepower ? parseFloat(maxHorsepower) : 1000;

      products = products.filter(
        (product) => product.horsepower >= minHP && product.horsepower <= maxHP
      );
    }

    // Apply legacy filters for compatibility (most filtering now done in Shopify query)
    if (status === "overstock" && !shopifyQuery.includes("tag:overstock")) {
      products = products.filter((product) =>
        product.tags.some((tag) => tag.toLowerCase() === "overstock")
      );
    }

    if (status === "sale" && !shopifyQuery.includes("tag:sale")) {
      products = products.filter((product) =>
        product.tags.some((tag) => tag.toLowerCase().includes("sale"))
      );
    }

    if (condition && !shopifyQuery.includes(`tag:${condition}`)) {
      products = products.filter(
        (product) =>
